'''
Program: Lesson 16: Step 2 and 3
Author: Cameron Roy
Date: 2020/05/22
Description: Now create the file, and write to it values that need to be saved and read. 
'''
...
# This is my weird way of making the file, I have it made with the operator +
# at the end of it, and in append mode, because as w+ it just blanks the file.
fR = open("recipie.rate", "a+")
fR.close()
...
while True:
    ...
    # This is the part where most of the fileIO will happen, here is where the program
    # reads the recipe.rate file for a previous rating of the recipie, and where it will
    # save the new rating of the recipie.
    def rating():
        # This reads out the file to see what the old rating of it was
        with open("recipie.rate") as file:
            # starsO is read from the file, and it is the old rating that the recipie
            # used to have. This is the part where it reads the file.
            starsO = file.read()
            print("Previous " + starsO)
        ...
        # the final rating is saved to the file for future reference.
        if (int(starsT) <= 5 and int(starsT) >= 0):
            starsF = starsT
            with open("recipie.rate","w") as file:
                file.write("rating: " + str(starsF))
    ...
    # Takes user input typecast as a string,  and runs it through the 
